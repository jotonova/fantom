...
// ── Step 2: Read journal ──────────────────────────────────────────────────────
const journal = JSON.parse(readFileSync(join(migrationsFolder, 'meta/_journal.json'), 'utf-8'));
...
try {
    const { rows: countRows } = await orphanClient.query('SELECT COUNT(*) AS count FROM "__drizzle_migrations"');
    const isEmpty = Number(countRows[0]?.count ?? 0) === 0;
    if (isEmpty) {
        const { rows: regRows } = await orphanClient.query("SELECT to_regclass('public.distributions') AS tbl");
        ...
        if (schemaExists) {
            ...
            console.log(`Detected pre-existing schema with empty tracking table. Backfilling ${toBackfill.length} migrations (0000-0007).`);
            for (const entry of toBackfill) {
                const sqlPath = join(migrationsFolder, `${entry.tag}.sql`);
                const sql = readFileSync(sqlPath, 'utf-8');
                const hash = createHash('sha256').update(sql).digest('hex');
                await orphanClient.query('INSERT INTO "__drizzle_migrations" (hash, created_at) VALUES ($1, $2)', [hash, entry.when]);
            }
        ...
        const { rows: maxRows } = await orphanClient.query('SELECT created_at FROM "__drizzle_migrations" ORDER BY created_at DESC LIMIT 1');
...
for (const entry of journal.entries) {
    ...
    try {
        ...
        await client.query('INSERT INTO "__drizzle_migrations" (hash, created_at) VALUES ($1, $2)', [hash, entry.when]);
